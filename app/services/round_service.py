...
from app.core.constants import GameMode, RoomStatus, RoundStatus
...
from app.schemas.room import StartRoundDTO
from app.schemas.submission import RoundPromptInfoDTO
from app.services.alias_service import generate_aliases
...
async def start_round(db: AsyncSession, room_id: str) -> StartRoundDTO:
    ...
    game_mode = room.gameMode or GameMode.confessions.value
    ...
    await generate_aliases(db, new_round)

    log_event(
        action="start_round",
        status="success",
        room_id=room_id,
        message=f"Round {new_round_number} started for mode {game_mode}",
    )

    await emit_room_state_updated(room_id)
    await emit_player_activity_updated(room_id)

    return StartRoundDTO(roundNumber=new_round_number)

async def get_round_prompt_info(
    db: AsyncSession, room_id: str, participant_id: str
) -> RoundPromptInfoDTO:
    round_obj = await get_current_round(db, room_id)
    if not round_obj:
        raise NotFoundException("No active round found.")
...
async def advance_to_guessing(db: AsyncSession, room_id: str) -> GameRound:
    round_obj = await get_current_round(db, room_id)
    if not round_obj:
        raise NotFoundException("No active round found to advance to guessing.")
