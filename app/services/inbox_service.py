...
from app.schemas.inbox import AnonymousChitDTO, AnonymousInboxDTO
...
class InboxService:
    ...
    async def get_anonymous_inbox(db: AsyncSession, room_id: str) -> AnonymousInboxDTO:
        ...
        # Sort anonymous cards by alias number for stable ordering
    ...
    @staticmethod
    async def mark_chit_read(db: AsyncSession, room_id: str, opaque_alias_id: str, is_read: bool) -> None:
        ...
        if not current_round:
            raise not_found("Round not found.")
    ...
    async def mark_chit_guessed(
        ...
        if chit:
            ...
            if guessed_alias_id:
                target_alias = await AliasService.resolve_opaque_alias_id(db, guessed_alias_id)
